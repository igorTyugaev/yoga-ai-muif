...
import {starDown, starUp} from "./meta";
...
export class AICamera {
    ...
    async predictionHand(detector, video) {
        if (!detector || !video) return;
        try {
            const predictions = await detector.estimateHands(this.video);
            const [prediction] = predictions;
            if (prediction) {
                const estimatedGestures = this.gestures.estimate(prediction.landmarks, 8.5);
                this.drawHand(predictions);
                if (estimatedGestures?.gestures) {
                    const [gesture] = estimatedGestures.gestures;
                }
            }
        } catch (error) {
            detector.dispose();
    ...
    async prediction(detector, video) {
        ...
        try {
            ...
            // const notDetected = pose && pose.keypoints[12].score < 0.3
    ...
    async loopPrediction() {
        ...
        // await this.predictionHand(this.handPoseDetector, this.video);
    ...
    angleRightHand(keypoints) {

        const shoulder = keypoints[POINTS.RIGHT_SHOULDER];
        const elbow = keypoints[POINTS.RIGHT_ELBOW];
        const wrist = keypoints[POINTS.RIGHT_WRIST];

        const avgScore = (shoulder.score + elbow.score + wrist.score) / 3
        if (avgScore < 0.56) return;
        // shoulder, elbow, wrist
        const se = Math.sqrt((elbow.x - shoulder.x) ** 2 + (elbow.y - shoulder.y) ** 2);
        const ew = Math.sqrt((wrist.x - elbow.x) ** 2 + (wrist.y - elbow.y) ** 2);
        const sw = Math.sqrt((wrist.x - shoulder.x) ** 2 + (wrist.y - shoulder.y) ** 2);
        const radians = Math.acos((se ** 2 + ew ** 2 - sw ** 2) / (2 * se * ew));
        let angle = Math.abs(radians * 180.0 / Math.PI)
        if (angle > 180.0) angle = 360 - angle
        if (angle > 130)
            this.stage = "down"
        else if (angle < 50 && this.stage === 'down') {
            this.stage = "up"
    ...
    drawResult(pose) {
        if (pose.keypoints != null) {
            ...
            // this.angleRightHand(pose.keypoints);
        }
    }
